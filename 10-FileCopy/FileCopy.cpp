...
BOOL FileCopy(PCTSTR pszFileSrc, PCTSTR pszFileDst) {
  ...
  try {
    {
      ...
      CEnsureCloseFile hFileSrc = CreateFile(
          pszFileSrc, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
      ...
      CEnsureCloseFile hFileDst =
          CreateFile(pszFileDst, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
      ...
      while ((nReadsInProgress > 0) || (nWritesInProgress > 0)) {
        ...
        DWORD dwNumBytes;
        CIOReq *pior;
        bResult = iocp.GetStatus(&CompletionKey, &dwNumBytes,
  ...
  if (bOk) {
    ...
    CEnsureCloseFile hFileDst =
        CreateFile(pszFileDst, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
    if (hFileDst.IsValid()) {
...
void Dlg_OnCommand(HWND hWnd, int id, HWND hWndCtl, UINT codeNotify) {
  ...
  case IDOK:
    ...
    Static_GetText(GetDlgItem(hWnd, IDC_SRCFILE), szPathname,
                   _countof(szPathname));
    SetCursor(LoadCursor(NULL, IDC_WAIT));
    chMB(FileCopy(szPathname, TEXT("FileCopy.cpy")) ? "File Copy Successful"
                                                    : "File Copy Failed");
    break;
  ...
  case IDC_PATHNAME:
    ...
    if (bOk) {
      ...
      CEnsureCloseFile hFile =
          CreateFile(szPathname, 0, 0, NULL, OPEN_EXISTING, 0, NULL);
      if (hFile.IsValid()) {
