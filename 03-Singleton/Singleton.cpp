...
void AddText(PCTSTR pszFormat, ...) {
  ...
  _vstprintf_s(_tcschr(sz, TEXT('\0')), _countof(sz) - _tcslen(sz), pszFormat,
               argList);
...
void CheckInstances() {
  ...
  if (!CreateWellKnownSid(WinBuiltinAdministratorsSid, NULL, pLocalAdminSID,
                          &cbSID)) {
  ...
  if (!ConvertStringSecurityDescriptorToSecurityDescriptor(
          TEXT("D:(A;;GA;;;BA)"), SDDL_REVISION_1, &sa.lpSecurityDescriptor,
          NULL)) {
